...
{
	sfVector2f pos = (sfVector2f){camera->scene_size.left, \
camera->scene_size.top};
...
{
	int min_x = camera->scene_size.left + camera->view_size.x / 2;
	int max_x = min_x + camera->scene_size.width - camera->view_size.x;
	int min_y = camera->scene_size.top + camera->view_size.y / 2;
...
{
	sfRectangleShape_setOutlineColor(shape, sfWhite);
	sfRectangleShape_setPosition(shape, camera->position);
